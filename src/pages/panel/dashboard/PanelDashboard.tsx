// Note : قسمت مدریت  کاربران هست که کاملا داینامیک یعنی با هافزودن ابجکت مورد نیاز به  ارایه ایتمز می تونیم یک مسیر نام ایکون دیگ حتی  با قابلیت انداز
...
export default function PanelDashboard() {
  ...
  // زاویه‌ها را به‌صورت یکنواخت برای آیتم‌ها تولید می‌کنیم
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[500px] h-full gap-6 p-4">
      <svg width="100%" viewBox="0 0 550 550" className="max-w-[500px]">
        ...
        >
          {hover !== null ? dynamicItems[hover].text : ""}
        </text>
        ...
        >
          {dynamicItems.map((item, i) => {
            ...
            const sy = center + centerCircleSize * Math.sin(rad);
            const ex = center + item.radius * Math.cos(rad);
            const ey = center + item.radius * Math.sin(rad);
            const iconSize = item.size * 0.5;
            const foSize = iconSize * 1.5;
            const foX = ex - foSize / 2;
            const foY = ey - foSize / 2;
            const isH = hover === i;
